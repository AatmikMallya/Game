...
// Traverses the voxel grid using the Amanatides & Woo algorithm while taking
float voxelRaytrace(vec3 origin, vec3 direction, out ivec3 grid_position) {
    ...
    ivec3 step_sign;
    step_sign.x = (direction.x > 0.0) ? 1 : ((direction.x < 0.0) ? -1 : 0);
    step_sign.y = (direction.y > 0.0) ? 1 : ((direction.y < 0.0) ? -1 : 0);
    step_sign.z = (direction.z > 0.0) ? 1 : ((direction.z < 0.0) ? -1 : 0);
    
    float tMaxX, tMaxY, tMaxZ;
    if (direction.x != 0.0) {
        if (step_sign.x > 0)
            tMaxX = t + (((float(grid_position.x) + 1.0) * scale - pos.x) / abs(direction.x));
        else
            tMaxX = t + ((pos.x - float(grid_position.x) * scale) / abs(direction.x));
    } else {
        tMaxX = 1e30;
    }
    if (direction.y != 0.0) {
        if (step_sign.y > 0)
            tMaxY = t + (((float(grid_position.y) + 1.0) * scale - pos.y) / abs(direction.y));
        else
            tMaxY = t + ((pos.y - float(grid_position.y) * scale) / abs(direction.y));
    } else {
        tMaxY = 1e30;
    }
    if (direction.z != 0.0) {
        if (step_sign.z > 0)
            tMaxZ = t + (((float(grid_position.z) + 1.0) * scale - pos.z) / abs(direction.z));
        else
            tMaxZ = t + ((pos.z - float(grid_position.z) * scale) / abs(direction.z));
    } else {
        tMaxZ = 1e30;
    }
    
    float tDeltaX = (direction.x != 0.0) ? (scale / abs(direction.x)) : 1e30;
    float tDeltaY = (direction.y != 0.0) ? (scale / abs(direction.y)) : 1e30;
    float tDeltaZ = (direction.z != 0.0) ? (scale / abs(direction.z)) : 1e30;
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        int index = grid_position.x + grid_position.y * gridWidth +
                    grid_position.z * gridWidth * gridHeight;
        ...
        if (tMaxX < tMaxY) {
            if (tMaxX < tMaxZ) {
                grid_position.x += step_sign.x;
                t = tMaxX;
                tMaxX += tDeltaX;
            } else {
                grid_position.z += step_sign.z;
                t = tMaxZ;
                tMaxZ += tDeltaZ;
            }
        } else {
            if (tMaxY < tMaxZ) {
                grid_position.y += step_sign.y;
                t = tMaxY;
                tMaxY += tDeltaY;
            } else {
                grid_position.z += step_sign.z;
